<div class="fixed inset-0 z-50 overflow-y-auto hidden" id="trashClassModal" aria-labelledby="trashClassModalLabel" aria-hidden="true">
        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full">
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="flex justify-between items-center pb-3 border-b">
                    <h3 class="text-lg font-medium text-gray-900" id="trashClassModalLabel">Danh sách lớp học đã xóa</h3>
                    <button type="button" class="text-gray-400 hover:text-gray-500"
                        onclick="modalHandler.close('trashClassModal')" aria-label="Close">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            @switch($class->class_type)
                                                @case('online')
                                                    ...
                                                    @break
                                                @case('offline')
                                                    <span class="bg-green-100 text-green-800 px-2 py-1 rounded-full text-xs">Offline</span>
                                                    @break
                                                @default
                                                    <span class="bg-purple-100 text-purple-800 px-2 py-1 rounded-full text-xs">Hybrid</span>
                                            @endswitch
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            {{ \Carbon\Carbon::parse($class->deleted_at)->format('d/m/Y H:i') }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <form action="{{ route('admin.classes.restore', $class->id) }}" method="POST" class="inline">
                                                @csrf
                                                <button type="submit"
                                                    class="text-green-600 hover:text-green-900 bg-green-100 hover:bg-green-200 p-1 rounded transition-colors duration-150"
                                                    title="Khôi phục"
                                                    onclick="return confirm('Bạn có chắc chắn muốn khôi phục lớp học này?')">
                ...
                <div class="flex justify-end pt-2 mt-4 border-t">
                    <button type="button" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded"
                        onclick="modalHandler.close('trashClassModal')">
                        Đóng
                    </button>
                </div>
            </div>
