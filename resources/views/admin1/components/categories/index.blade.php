...
@section('content')
    <main class="flex-grow">
        <div class="flex justify-between items-center p-2 border-bottom">
            <h1 class="text-2xl font-bold">Danh mục <span class="text-gray-500">({{ $pagination['total'] }})</span></h1>
            <div class="flex space-x-2">
                <button class="bg-blue-500 text-white px-2 py-1 rounded" onclick="modalHandler.open('createCategoryModal')">
                    <i class="fas fa-plus"></i> Thêm mới
                </button>
                <button class="border border-blue-500 text-blue-500 px-2 py-1 rounded" onclick="modalHandler.open('trashCategoryModal')">
                    <i class="fas fa-trash"></i> Xem danh mục đã xóa
                </button>
            </div>
        </div>
            ...
            <table class="min-w-full bg-white border border-gray-300">
                <thead>
                    <tr>
                        ...
                        <th class="border ps-1 py-1 border-gray-300 text-start">Số khóa học <i class="fas fa-sort"></i></th>
                ...
                <tbody>
                    @foreach ($categories as $key => $item)
                        <tr class="hover:bg-gray-100 transition-colors duration-150">
                            ...
                            <td class="ps-1 pt-1">{{ $item->courses_count ?? 0 }}</td>
                    ...
                    @if (count($categories) == 0)
                        <tr>
                            <td colspan="6" class="text-center ps-1 pt-1">Không có dữ liệu</td>
                        </tr>
                    @endif
                </tbody>
            </table>
...
@push('scripts')
    <script>
        function populateEditModal(item) {
            modalHandler.open('editCategoryModal');
            
            modalHandler.setEditModalData('editCategoryModal', {
                name: item.name,
                description: item.description,
                actionUrl: '{{ url('admin/categories') }}/' + item.id
            });
        }
    </script>
@endpush
